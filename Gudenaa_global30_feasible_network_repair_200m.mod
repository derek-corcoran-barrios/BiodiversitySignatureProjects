# Gudenaa_global30_feasible_network_repair_200m.mod
#
...
# approximately 30% final nature across the planning polygon as a whole and
# all five connected certificates. The four former subarea budgets are
# replaced by one lower and one tightly tolerant upper bound for the polygon.
#
...
param GlobalPolygonAreaHa > 0;
param GlobalCurrentNatureAreaHa >= 0;
param GlobalNatureTargetFraction > 0, <= 1 default 0.30;
param GlobalFinalNatureTargetHa >= 0;
param GlobalRestorationRequirementHa >= 0;
param GlobalRestorationToleranceHa >= 0 default 16;
...
# ---- Restoration and whole-polygon 30% budget ---------------------------
...
check:
    abs(GlobalPolygonAreaHa - sum {s in Subareas} SubareaAreaHa[s])
    <= 0.000001;
check:
    abs(
        GlobalCurrentNatureAreaHa
        - sum {s in Subareas} CurrentNatureAreaHa[s]
    ) <= 0.000001;
check:
    abs(
